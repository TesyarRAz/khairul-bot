...
func (h *ReminderWorkerHandler) SetupReminder() error {
	...
	for _, reminder := range reminders {
		...
		if reminder.Subuh {
			...
			(*h.scheduler).NewJob(gocron.OneTimeJob(gocron.OneTimeJobStartDateTime(schedule)), gocron.NewTask(h.internalRunReminder, reminder.ID, "subuh", schedule))
		}
		if reminder.Dzuhur {
			schedule := schedules[dayOfYear].Zuhr

			(*h.scheduler).NewJob(gocron.OneTimeJob(gocron.OneTimeJobStartDateTime(schedule)), gocron.NewTask(h.internalRunReminder, reminder.ID, "dzuhur", schedule))
		}
		if reminder.Ashar {
			schedule := schedules[dayOfYear].Asr

			(*h.scheduler).NewJob(gocron.OneTimeJob(gocron.OneTimeJobStartDateTime(schedule)), gocron.NewTask(h.internalRunReminder, reminder.ID, "ashar", schedule))
		}
		if reminder.Maghrib {
			schedule := schedules[dayOfYear].Maghrib

			(*h.scheduler).NewJob(gocron.OneTimeJob(gocron.OneTimeJobStartDateTime(schedule)), gocron.NewTask(h.internalRunReminder, reminder.ID, "maghrib", schedule))
		}
		if reminder.Isya {
			schedule := schedules[dayOfYear].Isha

			(*h.scheduler).NewJob(gocron.OneTimeJob(gocron.OneTimeJobStartDateTime(schedule)), gocron.NewTask(h.internalRunReminder, reminder.ID, "isya", schedule))
		}
	}
